...
default `cpp` backend, a native event sink captures compact semantic event data and serializes
one complete MessagePack batch at the once-per-iteration flush boundary. Python passes those
bytes directly to the publisher queue, so no Python callback, per-event Python object, or
Python serialization runs on this path. Pipeline parallelism and context parallelism are
rejected. Events are not published for draft models or during KV-cache-size estimation. When
streaming is enabled the buffered pull API returns an empty list rather than raising.
